...
function AdminTutors() {
  ...
  const emptyForm = {
    tutorName: "", email: "", phone: "",
    specialization: "", experience: "", departmentId: "", password: "",
  };
  ...
  useEffect(() => { fetchTutors(); fetchDepartments(); }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.tutorName || !form.email || !form.phone || !form.departmentId || !form.password) {
      setMessage("All fields are required."); setMsgType("error"); return;
    }
    setSubmitting(true); setMessage("");
    const payload = {
      tutorName: form.tutorName, email: form.email, phone: form.phone,
      specialization: form.specialization, experience: form.experience,
      departmentId: Number(form.departmentId), password: form.password,
    };
    try {
      await axios.post(rest.tutor, payload, getHeader());
      setShowModal(false); setForm(emptyForm); setMessage("");
      fetchTutors();
      showToast(`Tutor added! Credentials emailed to ${form.email}.`);
    } catch (err) {
      const msg = err.response?.data?.message || err.response?.data || "Failed to add tutor.";
      setMessage(String(msg).toLowerCase().includes("duplicate") || err.response?.status === 409
        ? "A tutor with this email or phone already exists." : String(msg));
      setMsgType("error");
    } finally {
      setSubmitting(false);
    }
  ...
  const openModal = () => { setForm(emptyForm); setMessage(""); setShowModal(true); };
    ...
    <div className="p-4" style={{ height: "calc(100vh - 70px)", overflowY: "auto" }}>
      ...
      {toastMsg && (
        <div style={{
          ...
          fontSize: "0.85rem", boxShadow: "0 4px 20px rgba(0,0,0,0.15)", maxWidth: 380,
        }}>
          {toastMsg}
        </div>
      )}
      ...
      <div className="row space-between items-center mb-4">
        <div>
          ...
          <p className="fs-p9 text-secondary">Add tutors — login credentials are emailed to them automatically.</p>
        </div>
        ...
        <div style={{
          ...
          {tutors.map((t) => (
            <div
              key={t.tutorId || t.id}
              className="card p-4"
              style={{ borderLeft: "4px solid var(--primary)", minWidth: 0 }}
            >
              {/* Avatar + Name */}
              <div className="row items-center mb-3" style={{ gap: 12 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p className="bold fs-p9" style={{ margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {t.tutorName}
                  </p>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {/* Email — full width to avoid overflow */}
                <div style={{ background: "var(--gray-100)", borderRadius: 6, padding: "7px 10px" }}>
                  <p className="fs-p8 text-secondary" style={{ marginBottom: 2 }}>Email</p>
                ...
                {/* Phone + Experience in a row */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                  <div style={{ background: "var(--gray-100)", borderRadius: 6, padding: "7px 10px" }}>
                    <p className="fs-p8 text-secondary" style={{ marginBottom: 2 }}>Phone</p>
                    <p className="bold fs-p9" style={{
                      margin: 0,
                      overflow: "hidden",
                      textOverflow: "ellipsis",
                  ...
                  <div style={{ background: "var(--gray-100)", borderRadius: 6, padding: "7px 10px" }}>
                    <p className="fs-p8 text-secondary" style={{ marginBottom: 2 }}>Experience</p>
                    <p className="bold fs-p9" style={{ margin: 0 }}>
                ...
                <div style={{ background: "var(--gray-100)", borderRadius: 6, padding: "7px 10px" }}>
                  <p className="fs-p8 text-secondary" style={{ marginBottom: 2 }}>Department</p>
                  <p className="bold fs-p9" style={{ margin: 0 }}>
                    {t.departmentModel?.departmentName || "—"}
                  </p>
                </div>
